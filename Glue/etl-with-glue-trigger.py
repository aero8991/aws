...
#testing files 
#M25-small_aws_v2.txt
#M25Reduced_for_test.txt
#89 items expected

s3job = boto3.resource('s3')
s3 = boto3.client('s3')

#data2 = s3.get_object(Bucket='medispan-poc', Key='Data/M25')
response = s3.list_objects_v2(Bucket='medispan-poc-genoa-demo', Prefix='sftp')
all = response['Contents'] 
key = max(all, key=lambda x: x['LastModified'])

data1 = s3.get_object(Bucket='medispan-poc-genoa-demo', Key=key.get('Key'))
print("----------------------------------------------------------------------------")
print("list objects")
print(response)
...
txt_file = s3job.Object('medispan-poc-genoa-demo', key.get('Key')).get()['Body'].read().decode('utf-8').splitlines()
...
df_ok_final['datetime'] = datetime.today().strftime('%Y-%m-%d')
df_ok_final['id'] = df_ok_final.index
...
output_file_csv = df_ok_final.to_csv(index=False)

tzinfo = timezone(timedelta(hours=-8))
#date time more specific 
currentDateTime = datetime.now(tzinfo).strftime("%m-%d-%Y %H-%M-%S %p")
#currentDateTime = datetime.now(tzinfo).strftime("%m-%d-%Y")

#s3.put_object(Body=output_file, Bucket='medispan-poc-genoa-demo', Key=f'output/M25Export-Records.json')

#s3.put_object(Body=output_file_parquet, Bucket='medispan-poc-genoa-demo', Key=f'output/M25Export-Records.parquet')
s3.put_object(Body=output_file_csv, Bucket='medispan-poc-genoa-demo', Key=f'output/{currentDateTime}-M25Export-Records.csv')
print('saved file to s3')

print("starting glue crawler")

glue_client = boto3.client('glue')
glue_client.start_crawler(Name='medispan-crawl-demo')
